...
def make_Delta_with_cont_spec(N1, Z, rho, a=-1.0, b=1.0, eps=1e-12):
    np.random.seed(0)
    H = np.random.rand(N1, N1) + 1j * np.random.rand(N1, N1)
    H = H + np.conj(H.T)

    Delta = np.zeros((Z.shape[0], H.shape[0], H.shape[1]), dtype=np.complex128)
    ...
    for i in range(en.shape[0]):
        for n in range(len(Z)):
            ...
            Delta[n, :, :] = Delta[n, :, :] + gn * vn[:, i] * np.conj(
...
dim = 3
H, Delta = make_Delta_with_cont_spec(dim, Z, semicircular)
...
def fit_cont_triqs(tol):
    ...
    # Gf interface
    bathhyb, bathenergy, delta_fit, final_error = hybfit_triqs(delta_iw, tol=tol, maxiter=500, debug=True)
    assert final_error < tol

    # BlockGf interface
    delta_blk = BlockGf(name_list=['up', 'down'], block_list=[delta_iw, delta_iw], make_copies=True)
    bathhyb, bathenergy, delta_fit, final_error = hybfit_triqs(delta_blk, tol=tol, maxiter=500, debug=True)
